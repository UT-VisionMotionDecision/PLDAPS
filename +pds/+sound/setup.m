function p = setup(p)
% function p = pds.sound.setup(p)   
% 
% loads audiofiles at the beginning of an experiment
...
%                  Fixed device id initialization to first found output index
% 
...
if p.trial.sound.use && isField(p.trial, 'pldaps.dirs.wavfiles')
    ...
    soundFiles = find(contains(soundDirFiles,'.wav'));    %(~cellfun(@isempty,strfind(soundDirFiles,'.wav')));
