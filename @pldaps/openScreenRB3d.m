...
if p.trial.datapixx.use
    ...
    PsychImaging('AddTask', 'General', 'FloatingPoint32Bit','disableDithering',1);
        ...
        if ~isField(p.trial.datapixx, 'videoMode')
            p.trial.datapixx.videoStatus.mode = 0; % default passthrough
            
            
        end
    
    if p.trial.display.useOverlay==1
        if p.trial.display.stereoMode == 1 && Datapixx('IsPropixx')
            % Modified overlay setup for RB3D overlay mode
            PsychImaging('AddTask', 'General', 'EnableProPixxRB3dWithOverlay');
            disp('Using Propixx RB3d overlay window (EnablePropixxRB3dWithOverlay)')
            disp('****************************************************************')
        else
...
% update Datapixx Video Status now that window has been opened
if p.trial.datapixx.use
...
if p.trial.display.useOverlay==1
    if p.trial.datapixx.use
        if p.trial.datapixx.videoStatus.mode~=9
            % do the normal overlay thing (PsychImaging has already done the rest)
            p.trial.display.overlayptr = PsychImaging('GetOverlayWindow', p.trial.display.ptr); % , dv.params.bgColor);
            
        else
            % is Propixx RB3d with overlay mode
            [wC, hC] = Screen('WindowSize', p.trial.display.ptr);
            [wF, hF] = Screen('WindowSize', p.trial.display.ptr, 1);
            sampleX = wC / wF;
            sampleY = hC / hF;
            % Build the shader (sourced from G channel, instead of normal bits++ R channel)
            shSrc = sprintf('uniform sampler2DRect overlayImage; float getMonoOverlayIndex(vec2 pos) { return(texture2DRect(overlayImage, pos * vec2(%f, %f)).g); }', sampleX, sampleY);
            % open offscreen window with indexed color range
